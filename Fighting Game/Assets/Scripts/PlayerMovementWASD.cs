...
public class PlayerMovementWASD : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        float horizontal = Input.GetAxisRaw("HorizontalWAD");
        ...
        if (isGrounded)
        {
            ...
            if (Input.GetKeyDown(KeyCode.E) && isGrounded && Time.time - lastAttackTime >= attackDelay)
            {
            ...
            if (Input.GetKeyDown(KeyCode.W) && isGrounded && !playerManager.isShiled)
            {
